...
/// FORK(review view): pane metadata token marking the reviewr pane this
/// sidebar opened, so the activity-bar chip can toggle it.
pub const TOKEN_REVIEW: &str = "hs-review-pane";

/// FORK(review view): toggle a `herdr-reviewr` pane in this tab. Open, it
/// closes; closed, it opens in the preview slot (the sidebar's inner side)
/// and the sidebar stays where it is. Errors are user-facing messages.
pub fn toggle_review(my_pane_id: &str, spawn_cwd: &Path) -> Result<(), String> {
    let list = ipc::call_text("pane.list", serde_json::json!({}))
        .map_err(|e| format!("review failed: {e}"))?;
    let my_tab = crate::launch::tab_of(&list, my_pane_id);
    if let Some(existing) = review_pane_in_tab(&list, &my_tab) {
        let _ = ipc::call_text("pane.close", serde_json::json!({ "pane_id": existing }));
        return Ok(());
    }
    let Some(bin) = crate::launch::resolve_reviewr() else {
        return Err("reviewr not found — herdr plugin install persiyanov/herdr-reviewr".into());
    };
    ...
    .map_err(|e| format!("review failed: {e}"))?;
    let new_pane = crate::launch::split_pane_id(&response)
        .ok_or_else(|| "review pane failed to open".to_string())?;
    if needs_swap
        && !ipc::call_text(
            "pane.swap",
        ...
        .is_ok_and(|response| ipc_succeeded(&response))
    {
        ...
        return Err("review pane could not be positioned".into());
    }
    let _ = ipc::call_text(
        "pane.report_metadata",
        serde_json::json!({
            "pane_id": new_pane,
            "source": "herdr-sidebar-review",
            "tokens": { TOKEN_REVIEW: "1" },
        }),
    );
    let _ = ipc::call_text(
        "pane.rename",
        serde_json::json!({ "pane_id": new_pane, "label": "review" }),
    );
    // exec: when the user quits reviewr the pane's process ends and herdr
    // reaps the pane, so `q` cleans up the whole slot.
    let command = format!("exec '{}'", bin.display());
    let _ = ipc::call_text(
        "pane.send_input",
...
/// FORK(review view): the reviewr pane this sidebar opened in `tab_id`.
fn review_pane_in_tab(pane_list_json: &str, tab_id: &str) -> Option<String> {
    ...
    let msg: Msg =
        serde_json::from_str(crate::launch::strip_bom(pane_list_json)).ok()?;
    msg.result
        .panes
        .into_iter()
        .find(|p| {
            p.tab_id.as_deref() == Some(tab_id) && p.tokens.contains_key(TOKEN_REVIEW)
        })
